...
void usage(const char *prog_name) {
    fprintf(stderr, "Usage: %s <wasm_file> test\n", prog_name);
    fprintf(stderr, "Example: %s wasm/print_prog.wasm test\n", prog_name);
}
...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < data.size; i++) {
        data.array[i] = i; // Example data values
    }
