...
/**
 * Dummy test call to ensure the function is linked
 */
uint32_t
init(void)
{
...
#define MY_PAGE_SIZE_LOG_2 16
#define PAGE_MASK (MY_PAGE_SIZE - 1)
STATIC_ASSERT_EQ(MY_PAGE_SIZE, 1 << MY_PAGE_SIZE_LOG_2);

#define CHUNKS_PER_PAGE 256
STATIC_ASSERT_EQ(MY_PAGE_SIZE, CHUNK_SIZE * CHUNKS_PER_PAGE);
...
allocate_pages(size_t payload_size, size_t *n_allocated) {
  ...
  size_t heap_size = __builtin_wasm_memory_size(0) * MY_PAGE_SIZE;
  ...
  if (!walloc_heap_size) {
    ...
    uintptr_t heap_base = align((uintptr_t)&__heap_base, MY_PAGE_SIZE);
  ...
  if (preallocated < needed) {
    ...
    grow = align(max(walloc_heap_size / 2, needed - preallocated),
                 MY_PAGE_SIZE);
    ASSERT(grow);
    if (__builtin_wasm_memory_grow(0, grow >> MY_PAGE_SIZE_LOG_2) == -1) {
      return MY_NULL;
    }
  ...
  ASSERT(size);
  ASSERT_ALIGNED(size, MY_PAGE_SIZE);
  *n_allocated = size / MY_PAGE_SIZE;
  return ret;
}
...
allocate_large_object(size_t size) {
  ...
  struct large_object *best = MY_NULL, **best_prev = &large_objects;
  size_t best_size = -1;
  ...
  if (!best) {
    ...
    best->size = best_size =
      n_allocated * MY_PAGE_SIZE - page_header - LARGE_OBJECT_HEADER_SIZE;
  ...
  *best_prev = next;
  ...
  if (tail_size) {
    ...
    if (start_page == get_page(end - tail_size - 1)) {
      ...
      best = (struct large_object *) ptr;
      best->size = best_size = best_size - first_MY_PAGE_SIZE - CHUNK_SIZE - LARGE_OBJECT_HEADER_SIZE;
      ...
      ASSERT_ALIGNED((uintptr_t)end, MY_PAGE_SIZE);
      size_t first_MY_PAGE_SIZE = MY_PAGE_SIZE - (((uintptr_t)start) & PAGE_MASK);
      size_t tail_pages_size = align(size - first_MY_PAGE_SIZE, MY_PAGE_SIZE);
      size = first_MY_PAGE_SIZE + tail_pages_size;
...
allocate_large(size_t size) {
  ...
  return obj ? get_large_object_payload(obj) : MY_NULL;
}
...
// test_allocation to show malloc/free usage
uint32_t test_mallocation(void) {
    void *b1 = (void*)(uintptr_t)malloc(32);
    void *b2 = (void*)(uintptr_t)malloc(64);
    void *b3 = (void*)(uintptr_t)malloc(32);

    if (b1) ((char*)b1)[0] = 42;
    if (b2) ((char*)b2)[0] = 99;
    if (b3) ((char*)b3)[0] = 7;

    if (b1) print_int(((char*)b1)[0]);
    if (b2) print_int(((char*)b2)[0]);
    if (b3) print_int(((char*)b3)[0]);
